const formResponse = {
    ...
    buildReport: function () {
        ...
        surveyHeader.addContent(
            new nep.bootstrap.Text({
                text: modelSurveyMaster.oData.survey.distribution.enableSurveyTitle ? modelSurveyMaster.oData.survey.name : modelSurveyData.oData.name,
                fontSize: "FontSize2",
            })
        );
        ...
        if (modelSurveyData.oData.description) {
            surveyHeader.addContent(
                new nep.bootstrap.Text({
                    text: modelSurveyMaster.oData.survey.distribution.enableSurveySubTitle ? modelSurveyMaster.oData.survey.description : modelSurveyData.oData.description,
                    type: "Paragraph",
                })
            );
        }
    ...
    buildToolbar: function (question, parent) {
        ...
        switch (question.type) {
            ...
            default:
                ...
                chartTypes.addItem(new sap.ui.core.Item({ key: "areaspline", text: "Area Spline" }));
    ...
    buildOverallChart: function (parent) {
        ...
        chartparent.onAfterRendering = function () {
            ...
            let options = formResponse.setChartDefaults(formResponse.overallCategory, series, question);
    ...
    calculateOverall: function (seriesData) {
        ...
        formResponse.overallSeries[seriesData.name].push(parseFloat((overallValue / overallCounter).toFixed(2)));
    },
    ...
    buildReportText: function (question, parent) {
        ...
        if (formResponse.anonymous) {
            // tableText.addColumn(new sap.m.Column({ width: "200px" }));
            tableText.addColumn(new sap.m.Column());
        } else {
            tableText.addColumn(new sap.m.Column({ width: "300px" }));
            // tableText.addColumn(new sap.m.Column({ width: "200px" }));
            tableText.addColumn(new sap.m.Column());
        }
        ...
        // Grouping by Group
        const oSorter1 = new sap.ui.model.Sorter("group", false, true);
        tableText.getBinding("items").sort(oSorter1);
    ...
    flatten: function (response) {
        ...
        resQuestions.forEach(function (keyQuestion) {
            ...
            const question = ModelData.FindFirst(modelSurveyData.oData.questions, "id", keyQuestion);
            ...
            if (!formResponse.response[question.id].group[response.groupid]) {
                if (question.items && (question.type === "SingleChoice" || question.type === "MultipleChoice")) {
                    count = {};
                    total = {};
                    question.items.forEach(function (item) {
                        count[item.id] = 0;
            ...
            switch (question.type) {
                case "Likert":
                    ...
                    resItems.forEach(function (keyItem) {
                        ...
                        if (!formResponse.response[question.id].group[response.groupid].items[item.id]) {
                            formResponse.response[question.id].group[response.groupid].items[item.id] = {
                                id: item.id,
                                title: item.title,
                                responseCount: 0,
                                count: [0, 0, 0, 0, 0],
                        ...
                        formResponse.response[question.id].group[response.groupid].items[item.id].responseCount++;
                        formResponse.response[question.id].group[response.groupid].items[item.id].count[resItem - 1]++;
                        formResponse.response[question.id].group[response.groupid].items[item.id].total[resItem - 1] += resItem;
                    });
                    break;
                ...
                case "MultipleChoice":
                    ...
                    resItems.forEach(function (keyItem) {
                        ...
                        formResponse.response[question.id].group[response.groupid].total[keyItem] += parseInt(resItem);
                    });
                    break;
                ...
                case "SingleChoice":
                    ...
                    formResponse.response[question.id].group[response.groupid].total[resData] += item.value;
                    break;

                case "NPS":
                    formResponse.response[question.id].group[response.groupid].responseCount++;
                    formResponse.response[question.id].group[response.groupid].count[resData]++;
                    formResponse.response[question.id].group[response.groupid].total[resData] += parseInt(resData);
                    break;

                default:
                    if (resData) {
                        formResponse.response[question.id].group[response.groupid].responseCount++;
                        formResponse.response[question.id].group[response.groupid].answer.push(resData);
                        formResponse.response[question.id].group[response.groupid].items.push(response.email);
                    }
                    break;
            }
        });
    ...
    generatePDF: function () {
        let pdfData = {
            survey: {
                title: modelSurveyMaster.oData.survey.distribution.enableSurveyTitle ? modelSurveyMaster.oData.survey.name : modelSurveyData.oData.name,
                subTitle: modelSurveyMaster.oData.survey.distribution.enableSurveySubTitle ? modelSurveyMaster.oData.survey.description : modelSurveyData.oData.description,
            },
            questions: [],
        };
        ...
        modelSurveyData.oData.questions.forEach((question) => {
            let responseCount = formResponse.response[question.id] ? formResponse.response[question.id].responseCount : 0;
